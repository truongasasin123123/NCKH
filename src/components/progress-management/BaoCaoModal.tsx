...
const BaoCaoModal: React.FC<BaoCaoModalProps> = ({
  ...
  mocOptions, baoCaoList, baoCaoFiles, setBaoCaoFiles, submittingBaoCao, onFinish,
}) => {
  return (
    <Modal
      ...
      footer={null}
    >
      <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ LoaiBaoCao: 'Theo mốc' }}>
        <Form.Item name="LoaiBaoCao" label="Loại báo cáo" rules={[{ required: true }]}>
          <Select
            ...
            onChange={(value: LoaiBaoCao) => { setLoaiBaoCao(value); form.setFieldsValue({ MaMoc: undefined, KyBaoCao: undefined }); }}
          />
        </Form.Item>
        {loaiBaoCao === 'Theo mốc' ? (
          <Form.Item name="MaMoc" label="Mốc tiến độ" rules={[{ required: true, message: 'Chọn mốc tiến độ' }]}>
            <Select
              ...
              disabled={!!editingBaoCao}
              options={(mocOptions || []).filter((moc) => editingBaoCao?.MaMoc === moc.MaMoc || !baoCaoList.some((report) => report.MaMoc === moc.MaMoc)).map((moc) => ({ value: moc.MaMoc, label: `${moc.ThuTu}. ${moc.TenMoc} (${moc.TrangThai})` }))}
            />
          </Form.Item>
        ) : (
