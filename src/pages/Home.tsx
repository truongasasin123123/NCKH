    ...
    <Layout style={{ background: '#f7f9f7', minHeight: '100vh' }}>
      <Content style={{ padding: 0 }}>
        <div style={{ overflow: 'hidden', background: '#fff' }}>
          <section style={{ background: 'linear-gradient(150deg,#0f4a28 0%,#1a6e3c 55%,#2d9e5f 100%)', minHeight: 'calc(100vh - 80px)', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, maxWidth: 720, padding: '64px 48px' }}>
              ...
              <Space wrap size={8}>
                ...
                {token && <Button size="large" ghost onClick={() => document.getElementById('lookup')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} icon={<SearchOutlined />} style={{ height: 50, borderRadius: 12, paddingInline: 28 }}>Tra cứu đề tài</Button>}
          ...
          {token && <section id="lookup" style={{ padding: '40px 24px 64px', background: '#f7f9f7' }}>
