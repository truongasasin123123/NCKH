...
/* ================== Loại tài liệu (danh mục quản lý) ================== */

export type NghiepVuLoai = 'dang_ky' | 'theo_doi' | 'nghiem_thu' | 'thanh_ly';

export interface NghiepVuAssignment {
  NghiepVu: NghiepVuLoai;
  BatBuoc: boolean;
}
...
export interface LoaiTaiLieu {
  ...
  DangSuDung: boolean;
  NgayTao: string;
  NghiepVuList: (NghiepVuAssignment & { Id: number })[];
}

export const getLoaiTaiLieuList = async (): Promise<LoaiTaiLieu[]> => {
  const res = await ApiAxios.get('/loai-tai-lieu');
  return res.data;
};

// Dùng ở các form upload: chỉ lấy loại tài liệu đang hoạt động và áp dụng đúng nghiệp vụ
export const getLoaiTaiLieuByNghiepVu = async (
  nghiepVu: NghiepVuLoai,
): Promise<(LoaiTaiLieu & { BatBuoc: boolean })[]> => {
  const res = await ApiAxios.get(`/loai-tai-lieu/nghiep-vu/${nghiepVu}`);
  return res.data;
};

export const createLoaiTaiLieu = async (payload: {
  TenLoaiTL: string;
  MoTa?: string;
  DangSuDung?: boolean;
  NghiepVuList: NghiepVuAssignment[];
}): Promise<LoaiTaiLieu> => {
  const res = await ApiAxios.post('/loai-tai-lieu', payload);
  return res.data;
};

export const updateLoaiTaiLieu = async (id: number, payload: Partial<{
  TenLoaiTL: string;
  MoTa: string;
  DangSuDung: boolean;
  NghiepVuList: NghiepVuAssignment[];
}>): Promise<LoaiTaiLieu> => {
  const res = await ApiAxios.patch(`/loai-tai-lieu/${id}`, payload);
  return res.data;
};

export const deleteLoaiTaiLieu = async (id: number) => {
  const res = await ApiAxios.delete(`/loai-tai-lieu/${id}`);
  return res.data;
};